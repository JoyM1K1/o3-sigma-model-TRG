...
void Trace(const int merge_point, double const K, int const D_cut, int const n_node, int const N, std::ofstream &file) {
    ...
    for (int n = 1; n <= N; ++n) {
        ...
        HOTRG::renormalization::mass_alt(T, IMT, orders, n, merge_point, NORMALIZE_FACTOR, res);
...
int main(int argc, char *argv[]) {
    ...
    const string dir = "../data/gauss_quadrature/HOTRG_mass_alt/beta" + ss.str() + "/N" + std::to_string(N) + "/node" + std::to_string(n_node) + "/D" + std::to_string(D_cut) + "/";
    time_counter time;
    string fileName;
