...
#pragma omp parallel for default(none) private(n, i, j, k, l, two_i, two_j, two_k, two_l, im, jm, km, lm, two_im, two_jm, two_km, two_lm, L, M, two_L, two_M, c, sum, a) shared(l_max, A, T) schedule(static, 1)
...
#pragma omp parallel for default(none) private(n, i, j, k, l, two_i, two_j, two_k, two_l, im, jm, km, lm, two_im, two_jm, two_km, two_lm, L, M, L_, M_, two_L, two_M, two_L_, two_M_, m, two_m, c, sum, a) shared(l_max, A, IMT) schedule(static, 1)
    for (n = 0; n < (l_max + 1) * (l_max + 1) * (l_max + 1) * (l_max + 1); ++n) {
        i = n % (l_max + 1);
        j = n / (l_max + 1) % (l_max + 1);
        k = n / (l_max + 1) / (l_max + 1) % (l_max + 1);
